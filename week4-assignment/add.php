...
if (isset($_POST['first_name']) && isset($_POST['last_name']) && isset($_POST['email']) && isset($_POST['headline']) && isset($_POST['summary'])) 
{
    ...
    for($i=1; $i<=9; $i++) {
        ...
        $year_edu = $_POST['year_edu'.$i];
        $school = $_POST['school'.$i];

        // Lookup the institution name if it is already saved
        $institution_id = false;
        $stmt3 =$pdo->prepare('SELECT institution_id FROM institution WHERE name = :name');
        $stmt3->execute(array(':name' => $school));
        $row = $stmt3->fetch(PDO::FETCH_ASSOC);
        if ( $row !== false ) $institution_id = $row['institution_id'];
        // If institution is not found, insert it
...
$(document).ready(function(){
    ...
    $('#addEdu').click(function(event){
        ...
        $('#education_fields').append(
            ...
            <p>School: <input type="text" name="school'+countEdu+'" class="school" size="60" value="" /> \
            </div>');
